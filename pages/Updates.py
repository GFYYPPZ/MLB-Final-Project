...
if ('customer' in st.session_state) and ('feedback' in st.session_state) and ('orders_overall' in st.session_state) and ('products' in st.session_state):
    ...
    with st.container(border=True):
        ...
        # Calculate best selling product categories by quantity
        category_ranking = merged_data.groupby('Product_Category')['Quantity'].sum().sort_values(ascending=False).reset_index()
        
        # Calculate best selling food types by quantity
        food_type_ranking = merged_data.groupby('Food_type')['Quantity'].sum().sort_values(ascending=False).reset_index()

        # st.header("Top 3 Best Selling Products")
        # st.dataframe(top_products.style.format({'Total_Sales': "${:,.2f}"}))

        # st.header("Best Selling Product Categories")
        # st.dataframe(category_ranking)

        # st.header("Best Selling Food Types")
        # st.dataframe(food_type_ranking)

        # Top 3 Best Selling Products Plot
        st.header("Top 3 Best Selling Products")
